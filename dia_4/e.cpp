...
void solve(){
    ...
    forn(i,n){
        int l;cin>>l;
        dists[l] = make_pair(0,n);
        // cout<<"l: "<<l<<abs(dists[l].first)<<" "<<dists[l].second<<el;
    }
    fora(i,1,k)
    {
        dists[i] = make_pair(max(abs(dists[i].first-i),dists[i].first),max(dists[i].second-i, dists[i].second));
        cout<<"l: "<<i<<dists[i].first-i<<" "<<dists[i].second-i<<el;

    }    // for(const auto& m: dists) cout<<"Baldosa: "<<m.first <<" "<<m.second.first<<m.second.se;
}
