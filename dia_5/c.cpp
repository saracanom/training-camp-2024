...
int buscar(int i, int j, vector<vector<bool> >& visited, vector<vector<int> >& lake, int n, int m, int cont){
    // cout<<i<<j<<el;

    if (i>=n || j>=m || i<0 || j <0 || visited[i][j] == true || lake[i][j]<=0){
        return cont;
    }
    else{
        
        cont += lake[i][j];
        // cout<<i<<j<<visited[i][j]<<el;
        visited[i][j] = true;
        cont = buscar(i, j+1, visited , lake, n,m, cont);
        cont = buscar(i, j-1, visited , lake, n,m, cont);
        cont = buscar(i+1, j, visited , lake, n,m, cont);
        cont = buscar(i-1, j, visited , lake, n,m, cont);
...
void solve(){
    ...
    forn(i,n){
        forn(j,m){
            // cout<<i<<j<<" "<<maxi<<el;
            maxi = max(maxi, buscar(i,j, visited, lake , n, m, 0));
